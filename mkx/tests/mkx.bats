...
@test "shows version" {
	local version="$(grep VERSION "$BATS_TEST_DIRNAME/../mkx" | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+')"

	run "$BATS_TEST_DIRNAME/../mkx" -v
...
@test "generates script from custom template" {
	OUTPUT_FILE="$(mktemp)"
	rm -f "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -t default "$OUTPUT_FILE"
...
@test "searches for template in template directory adjacent to script" {
	...
	cp "$BATS_TEST_DIRNAME/../mkx" "$OUTPUT_DIR/mkx"
	mkdir -p "$OUTPUT_DIR/templates"
	cp -r "$BATS_TEST_DIRNAME/../lib" "$OUTPUT_DIR/lib"
	cp "$BATS_TEST_DIRNAME/../templates/default.mustache" "$OUTPUT_DIR/templates/custom.mustache"
...
@test "searches for template in system share templates directory" {
	...
	cp "$BATS_TEST_DIRNAME/../mkx" "$OUTPUT_DIR/bin/mkx"
	cp -r "$BATS_TEST_DIRNAME/../lib" "$OUTPUT_DIR/lib"
	cp "$BATS_TEST_DIRNAME/../templates/default.mustache" "$OUTPUT_DIR/share/mkx/templates/custom.mustache"
...
@test "searches for template in custom directory" {
	...
	cp "$BATS_TEST_DIRNAME/../mkx" "$OUTPUT_DIR/bin/mkx"
	cp -r "$BATS_TEST_DIRNAME/../lib" "$OUTPUT_DIR/lib"
	cp "$BATS_TEST_DIRNAME/../templates/default.mustache" "$MKX_TEMPLATES_DIR/custom.mustache"
...
@test "fails to generate script from non-existing template" {
	OUTPUT_FILE="$(mktemp)"
	rm -f "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -t non_existing_template "$OUTPUT_FILE"
...
@test "generates base script with /bin/sh as interpreter" {
	OUTPUT_FILE="$(mktemp)"
	rm -f "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -s "$OUTPUT_FILE"
...
@test "overwrites existing file when -f is set" {
	...
	rm -f "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -b "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -b -s -f "$OUTPUT_FILE"
...
@test "makes existing file executable with -x" {
	...
	touch "$OUTPUT_FILE"
	chmod -w "$OUTPUT_FILE"
	run "$BATS_TEST_DIRNAME/../mkx" -x "$OUTPUT_FILE"
...
@test "shows help when an invalid option is set" {
	run bash -c "'$BATS_TEST_DIRNAME/../mkx' -h 2>/dev/null"
	local help="$output"

	run bash -c "'$BATS_TEST_DIRNAME/../mkx' -t 2>/dev/null"
...
@test "exits with the right exit code" {
	...
	run "$BATS_TEST_DIRNAME/../mkx" -t non_existing_template
	[ "$status" -eq 16 ]

	run "$BATS_TEST_DIRNAME/../mkx"
	[ "$status" -eq 0 ]

	run "$BATS_TEST_DIRNAME/../mkx" -b
	[ "$status" -eq 16 ]

	run "$BATS_TEST_DIRNAME/../mkx" -s
	[ "$status" -eq 16 ]
}
