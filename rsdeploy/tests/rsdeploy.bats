...
@test "rsdeploy resolves config paths from the working directory" {
	TMP_DIR="$(mktemp -d)"
	mkdir -p "$TMP_DIR/project/config/example.com" "$TMP_DIR/project/dist" "$TMP_DIR/bin"

	printf '%s\n' \
		'RSDEPLOY_DEST=example.com:/site/' \
		'RSDEPLOY_FILTER=config/example.com/deploy-filter' \
		'RSDEPLOY_SRC=dist/' > "$TMP_DIR/project/config/example.com/env"
	touch "$TMP_DIR/project/config/example.com/deploy-filter"
	printf '%s\n' '#!/bin/sh' '[ "$PWD" = "$RSDEPLOY_TEST_PWD" ]' > "$TMP_DIR/bin/rsync"
	chmod +x "$TMP_DIR/bin/rsync"

	run bash -c 'cd "$1" && PATH="$2:$PATH" RSDEPLOY_TEST_PWD="$PWD" "$3" -c "$4"' _ \
		"$TMP_DIR/project" "$TMP_DIR/bin" "$BATS_TEST_DIRNAME/../rsdeploy" "config/example.com/env"
